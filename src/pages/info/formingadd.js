...
export default function FormingAdd() {
    const seasons = [
    ...
    const { infoformadd, infoformupdate } = GetPaths();
    ...
    const handleSubmit = (event) => {
        ...
        handleAPICall('post', isEdit ? infoformupdate : infoformadd, null, newObj, null).then(res => {
            if (res.type) 
                navigate(-1);
